...
def load_main_and_metadata_files(temp_dir):
    ...
    main_files = [f for f in os.listdir(temp_dir) if f.startswith("API") and f.endswith(".csv")]
    metadata_files = [f for f in os.listdir(temp_dir) if f.startswith("Metadata_Indicator") and f.endswith(".csv")]
...
def clean_and_merge_data(main_file, metadata_file, temp_dir, year_columns):
    ...
    # Load main data
    main_df = pd.read_csv(os.path.join(temp_dir, main_file), encoding="utf-8-sig", skiprows=4)
    main_df.fillna(0, inplace=True)
    main_df = main_df.loc[:, ~main_df.columns.str.contains("^Unnamed")]

    # Load metadata
    metadata_df = pd.read_csv(os.path.join(temp_dir, metadata_file), encoding="utf-8-sig")
...
def main():
    ...
    for main_file, metadata_file in zip(main_files, metadata_files):
        merged_df = clean_and_merge_data(main_file, metadata_file, temp_dir, year_columns)
